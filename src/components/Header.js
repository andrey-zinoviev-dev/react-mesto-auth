...
function Header(props) {
    ...
    return (
        <header className="header">
            {showUser && hideUser ? <div className="header__showedUser"><span>{user ? user.email : ""}</span><a className="header__action header__action_loggedIn" href={loginType ? "/sign-up" : "/sign-in"} onClick={props.loggedIn ? handleLogout: ""}>{props.loggedIn ? "Выйти" : loginType ? "Регистрация" : "Войти"}</a></div> : ""}
            <div className="header__wrapper">
                ...
                <div className="header__user-action">
                    ...
                    {!hideUser ? <span className="header__user-action-email">{user ? user.email : ""}</span> : ""}
